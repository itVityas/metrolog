...
class VerificationPersonMigrateView(LoginRequiredMixin, View):
    model = VerificationPerson
    success_url = reverse_lazy('verification_person')

    def post(self, request, *args, **kwargs):
        table = DBF('dbf/mb011.DBF')

        data_list = []
        for record in table:
            new_dict = {}
            new_dict['code'] = record.get('KOD_POV')
            new_dict['rank'] = record.get('RAZR')
            new_dict['fio'] = record.get('FIO')
            new_dict['is_active'] = True
            data_list.append(new_dict)

        obj_list = [VerificationPerson(**data_dict) for data_dict in data_list]
        VerificationPerson.objects.bulk_create(obj_list)
