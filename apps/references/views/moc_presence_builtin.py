...
class MocPresenceBuiltinView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        latest_device_station_query = pmodels.DeviceStation.objects.filter(
            moc_list=OuterRef('pk')
            ).order_by('-id')
                ...
                'device_status_date').annotate(
                    ...
                    last_verification_date=Subquery(
                        latest_verification_info_query.values(
                            'verification_date')[:1]),
                    last_device_station_inv_number=Subquery(
                        latest_device_station_query.values(
                            'station_inv_number')[:1]),
                    ).filter(
                        change_type__in=change_type_list,
                        last_location=department,
