...
class DeviceStatusMigrateView(LoginRequiredMixin, View):
    model = DeviceStatus
    success_url = reverse_lazy('device_status')

    def post(self, request, *args, **kwargs):
        table = DBF('dbf/mb003.DBF')

        data_list = []
        for record in table:
            new_dict = {}
            new_dict['name'] = record.get('NST')
            data_list.append(new_dict)

        obj_list = [DeviceStatus(**data_dict) for data_dict in data_list]
        DeviceStatus.objects.bulk_create(obj_list)
